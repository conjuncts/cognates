# INSTRUCTIONS
# to update LANGCODES: 
# go to https://en.wiktionary.org/wiki/Module:languages/code_to_canonical_name. Then apply the regex: 
# REPLACE \[(".*")\] = (".*"),\n WITH $1: $2,\n

# to update ETYCODES: 
# go to https://en.wiktionary.org/wiki/Module:etymology_languages/data
# LIST m\[(".*")] = {.*\n\t(?:--.*\n\t)?(".*") WITH $1: $2,\n

# to update FAMILIES: 
# https://en.wiktionary.org/wiki/Wiktionary:List_of_families
# go to https://en.m.wiktionary.org/wiki/Module:families/data
# LIST m\[(".*")] = {.*\n\t(?:--.*\n\t)?(".*") WITH $1,\n

# https://en.wiktionary.org/w/index.php?title=Module%3Alabels%2Fdata%2Flang%2Fla&action=edit
# Etymology-only languages also have aliases: (and apparently also language families)
# https://en.wiktionary.org/wiki/Template:borrowed

# There are a small number of exceptions, mostly representing Latin codes (ie. LL.)
# https://en.m.wiktionary.org/wiki/Module:etymology_languages/track-bad-etym-code
# https://simple.wiktionary.org/wiki/Module:etymology_languages/code_to_canonical_name
# https://en.m.wiktionary.org/wiki/Module:families/data says that ira-mid and ira-old are also exceptions
EXCEPTIONS = {
	...
	# these ones that are in the middle of deprecation
	"ira-mid": "Middle Iranian",
	"ira-old": "Old Iranian",
    "prv": "Provençal",
    "gkm-med": "Medieval Greek",
}
...
"gsw-FR": "Alsatian Alemannic German",
...
def is_langcode(code):
    return code in LANGCODES or code in ETYCODES or code in EXCEPTIONS or code in FAMILIES

def langcode_to_name(code):
    if code in LANGCODES:
        return LANGCODES[code]
    if code in ETYCODES:
        return ETYCODES[code]
